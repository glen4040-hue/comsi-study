...
function renderQuestions(){
  let s=$('search').value.trim().toLowerCase(),r=$('roundFilter').value,se=$('sessionFilter').value,c=$('catFilter').value,sort=$('sortFilter').value;
  let d=all().filter(q=>(!s||q.text.toLowerCase().includes(s))&&(!r||q.round==r)&&(!se||q.session==se)&&(!c||q.category===c)&&(!favOnly||q.favorite));
  ...
  $('questionList').innerHTML=d.slice(0,300).map(q=>`<article class="qitem" data-id="${q.id}">
    ...
    <div class="qbottom"><span class="tag">${escapeHtml(q.category)}</span><span class="chevron">›</span></div>
...
['search','roundFilter','sessionFilter','catFilter','sortFilter'].forEach(id=>$(id).addEventListener(id==='search'?'input':'change',renderQuestions));
...
function openDetail(id){let q=all().find(x=>x.id===id);if(!q)return;currentId=id;$('modalMeta').textContent=`제${q.round}회 · ${q.session}교시 · ${q.number}번`;$('modalTitle').textContent=`${q.number}번 문제`;$('modalQuestion').textContent=q.text;$('editCategory').value=q.category;$('editFavorite').checked=!!q.favorite;$('editModel').value=q.modelAnswer||'';$('editMine').value=q.myAnswer||'';$('editMemo').value=q.memo||'';$('modal').classList.add('open');document.body.style.overflow='hidden';}
...
$('saveDetail').onclick=()=>{user[currentId]={...(user[currentId]||{}),category:$('editCategory').value,favorite:$('editFavorite').checked,modelAnswer:$('editModel').value,myAnswer:$('editMine').value,memo:$('editMemo').value};save();closeDetail();renderQuestions();};
...
if('serviceWorker'in navigator&&location.protocol!=='file:')navigator.serviceWorker.register('sw.js?v=12');
